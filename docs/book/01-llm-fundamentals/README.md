...
**Простое решение (не рекомендуется):** Обрезка истории
```go
// Обрезка истории (оставляем только последние N сообщений)
// ПРОБЛЕМА: Теряем важную информацию из начала разговора!
...
**Лучшее решение: Сжатие контекста через саммаризацию**
...
- [ ] История диалога управляется (сжимается через саммаризацию или обрезается при переполнении)
