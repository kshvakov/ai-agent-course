...
Если вы хотите перейти от "учебного агента" к "прод-агенту", сфокусируйтесь на направлениях ниже. Часть тем уже раскрыта в учебнике, часть — это практические вопросы, которые почти всегда появляются в реальных системах.

**📖 Подробное раскрытие всех тем:** [Глава 12: Углублённое изучение](./docs/book/12-advanced-study/README.md)

### Качество и надёжность (обязательный прод-блок)

- **Evals и регрессии**: [Глава 09: Evals и Надежность](./docs/book/09-evals-and-reliability/README.md)
- **Прод-готовность и эксплуатация**: [Глава 11: Best Practices](./docs/book/11-best-practices/README.md)
- **Метрики качества**: pass rate, error rate, latency, token usage, iterations per task — см. [Observability и Tracing](./docs/book/12-advanced-study/README.md#observability-и-tracing)
- **Наборы тестов (datasets)**: хранение "золотых" сценариев, flaky-кейсы, расширение набора при инцидентах — см. [Evals в CI/CD](./docs/book/12-advanced-study/README.md#evals-в-cicd)
- **CI/CD для промптов**: автозапуск evals, quality gates, отчёты по метрикам — см. [Evals в CI/CD](./docs/book/12-advanced-study/README.md#evals-в-cicd)

### Безопасность и governance (обязательный прод-блок)

- **Confirmation/Clarification и guardrails**: [Глава 06: Безопасность и Human-in-the-Loop](./docs/book/06-safety-and-hitl/README.md)
- **Threat modeling для tool-агентов**: какие действия считаем критическими, как оцениваем риск — см. [Безопасность и Governance](./docs/book/12-advanced-study/README.md#безопасность-и-governance-обязательный-прод-блок)
- **Prompt injection / data exfiltration**: правила обработки входных данных, разделение ролей и контекстов — см. [Безопасность и Governance](./docs/book/12-advanced-study/README.md#безопасность-и-governance-обязательный-прод-блок)
- **Sandbox / dry-run режимы**: ограничение прав, политика доступа к инструментам — см. [Безопасность и Governance](./docs/book/12-advanced-study/README.md#безопасность-и-governance-обязательный-прод-блок)
- **Аудит**: журналирование вызовов инструментов и результатов, трассировка решений — см. [Безопасность и Governance](./docs/book/12-advanced-study/README.md#безопасность-и-governance-обязательный-прод-блок)

### Архитектура, рантайм и масштабирование

- **Память, планирование, рантайм**: [Глава 03: Анатомия Агента](./docs/book/03-agent-architecture/README.md)
- **Инструменты и контракт tool↔runtime**: [Глава 04: Инструменты и Function Calling](./docs/book/04-tools-and-function-calling/README.md)
- **Автономный цикл и остановки**: [Глава 05: Автономность и Циклы](./docs/book/05-autonomy-and-loops/README.md)
- **Observability и tracing**: трейсинг agent run, span'ы tool calls, метрики latency/token/errors, кореляция с id запроса — см. [Observability и Tracing](./docs/book/12-advanced-study/README.md#observability-и-tracing)
- **Контекстные бюджеты**: токены, "слишком длинные" tool results, компрессия/саммаризация — см. [Cost & Latency Engineering](./docs/book/12-advanced-study/README.md#cost--latency-engineering)
- **Параллелизм**: несколько tool calls, управление временем ожидания, retries/backoff, идемпотентность — см. [Workflow и State Management](./docs/book/12-advanced-study/README.md#workflow-и-state-management)

### RAG и работа со знаниями (если у вас есть документация/процедуры)

- **RAG протокол, чанкинг, retrieval**: [Глава 07: RAG и База Знаний](./docs/book/07-rag/README.md)
- **Качество retrieval**: coverage/precision, "не нашли" vs "нашли не то" — см. [RAG в продакшене](./docs/book/12-advanced-study/README.md#rag-в-продакшене)
- **Реранкинг и фильтрация**: устранение мусора, приоритизация источников — см. [RAG в продакшене](./docs/book/12-advanced-study/README.md#rag-в-продакшене)
- **Grounding**: запрещаем "выдумывать", обязуем ссылаться на найденное — см. [RAG в продакшене](./docs/book/12-advanced-study/README.md#rag-в-продакшене)
- **Обновление базы знаний**: версии документов, устаревание, правила доверия — см. [RAG в продакшене](./docs/book/12-advanced-study/README.md#rag-в-продакшене)

### Multi-Agent (если инструментов много или задачи разнородные)

- **Supervisor/Worker, изоляция контекста**: [Глава 08: Multi-Agent Systems](./docs/book/08-multi-agent/README.md)
- **Маршрутизация задач**: кто принимает решение, кто исполняет, как агрегировать результаты — см. [Multi-Agent в продакшене](./docs/book/12-advanced-study/README.md#multi-agent-в-продакшене)
- **Контуры безопасности**: какие Workers имеют какие инструменты и права — см. [Multi-Agent в продакшене](./docs/book/12-advanced-study/README.md#multi-agent-в-продакшене)
- **Наблюдаемость**: трассировка цепочки Supervisor → Worker → tool — см. [Multi-Agent в продакшене](./docs/book/12-advanced-study/README.md#multi-agent-в-продакшене)

### Модель и декодинг (то, что ломает агентов чаще всего)

- **Capability benchmark** (проверка модели до разработки): [Приложение](./docs/book/appendix/README.md) — см. также [Модель и декодинг](./docs/book/12-advanced-study/README.md#модель-и-декодинг-то-что-ломает-агентов-чаще-всего)
- **Детерминизм**: `Temperature = 0` для tool calling/JSON — см. [Модель и декодинг](./docs/book/12-advanced-study/README.md#модель-и-декодинг-то-что-ломает-агентов-чаще-всего)
- **Structured outputs / JSON mode** (если модель поддерживает): снижение "ломаного" JSON — см. [Модель и декодинг](./docs/book/12-advanced-study/README.md#модель-и-декодинг-то-что-ломает-агентов-чаще-всего)
- **Выбор модели под задачу**: стоимость/скорость/контекст/качество следования инструкциям — см. [Модель и декодинг](./docs/book/12-advanced-study/README.md#модель-и-декодинг-то-что-ломает-агентов-чаще-всего)

### Справочники и шаблоны

- **Глоссарий, чек-листы, SOP, таблицы решений**: [Приложение](./docs/book/appendix/README.md)
