...
Промпт — это код для нейросети. Только, в отличие от обычного кода, он работает через управление вероятностями.
...
В этой главе разберём, как писать промпты так, чтобы они реально управляли поведением агента.
...
**Решение:** Нормальный System Prompt с SOP (Standard Operating Procedure) задаёт порядок действий: статус → логи → анализ → фикс → верификация.
...
Формат ответа:
- Если нужно вызвать инструмент — используй Function Calling
...
> **Важно: Few-shot задаёт не только формат, но и скрытые подсказки.**
...
> **Важно: CoT может быть post-hoc (после факта).**
...
**Сдано:**
- [x] Роль (Persona) четко определена
- [x] Цель (Goal) конкретна и измерима
- [x] Ограничения (Constraints) явно указаны
- [x] Формат ответа (Format) описан
- [x] SOP (если применимо) детально расписан
- [x] CoT включен для сложных задач
- [x] Few-Shot примеры добавлены (если нужен сложный формат)
- [x] Few-Shot примеры согласованы (один формат)
- [x] Few-Shot примеры разнообразны (нет позиционного биаса)
- [x] Промпт не подсказывает желаемый ответ (нейтральные формулировки)
- [x] Предпочтения пользователя отделены от фактов
- [x] Ответы проверяются через инструменты и evals, не только через CoT
