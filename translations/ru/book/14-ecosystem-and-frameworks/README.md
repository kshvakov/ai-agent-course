...
> **Дисклеймер:** агентные фреймворки эволюционируют быстро — мажорные релизы выходят раз в несколько месяцев, API ломается, сильные/слабые стороны меняются. Сводка ниже — ориентир на момент написания. Перед выбором проверяйте свежую документацию, последние релизы и реальные production-кейсы (issue tracker, downgrade-репорты).
...
5. **Простота вместо моды.** Многие «продвинутые» паттерны из обзоров и постов про агентов на практике не нужны. Линейная история сообщений, один `condense` при переполнении окна, фиксированный system prompt, лимит итераций, защита от повторов — этого достаточно для большинства задач (см. [гл. 12](../12-agent-memory/README.md), [гл. 13](../13-context-engineering/README.md)). Многоуровневые сжатия, скоринг важности сообщений, динамические промпты, тяжёлые «фреймворки памяти» обычно решают не ту проблему: они дороги, ломают prompt cache и усложняют отладку, при этом редко окупаются на реальных нагрузках. Это не значит, что они вредны всегда — но прежде чем их внедрять, стоит проверить базовые механизмы и убедиться, что простого решения действительно не хватает. Когда вы пишете runtime сами, эту разницу видно сразу.
...
- [x] Знаете, как избежать vendor lock-in через свои интерфейсы (Tool, Memory, AgentLoop).
- [x] Можете оценить фреймворк против ваших требований и спокойно отнестись к громким обещаниям про «революционные» паттерны.
- [x] Понимаете общие паттерны, которые повторяются во всех фреймворках (Tool Registry, Agent Loop, Memory).
- [x] При выборе фреймворка проверяете свежесть данных (релизы, issues, реальные production-отзывы).

**Не сдано:**
- [ ] Выбор фреймворка без оценки требований.
- [ ] Тесная связь с API фреймворка (типы фреймворка везде в бизнес-логике).
- [ ] Нет пути миграции, если фреймворк не подойдёт.
- [ ] Игнорирование ограничений фреймворка («заставлю его делать то, для чего он не предназначен»).
- [ ] Слепое копирование «продвинутых» паттернов из чужих репозиториев и блог-постов без понимания, какую конкретно проблему они решают.
...
- **[Глава 12: Системы Памяти Агента](../12-agent-memory/README.md)** и **[Глава 13: Context Engineering](../13-context-engineering/README.md)** — Простая модель памяти и контекста, к которой стоит примерять любой фреймворк, прежде чем брать его как «продвинутое» решение.
- **[Глава 18: Протоколы Инструментов и Tool Servers](../18-tool-protocols-and-servers/README.md)** — Стандартные протоколы (MCP, A2A) уменьшают vendor lock-in.
