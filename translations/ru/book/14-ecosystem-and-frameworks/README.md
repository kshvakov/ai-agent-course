...
**Кастомный Runtime:**
- [x] Полный контроль над каждым компонентом
- [x] Нет vendor lock-in
- [x] Оптимизирован под ваш конкретный use case
- [ ] Больше кода для написания и поддержки
- [ ] Нужно реализовывать общие паттерны самостоятельно

**Фреймворк:**
- [x] Быстрая разработка, проверенные паттерны
- [x] Поддержка сообщества и примеры
- [x] Обрабатывает edge cases, которые вы можете пропустить
...
Преимущество MCP — один протокол для всех инструментов. Агент подключает MCP-сервер и получает доступ к его инструментам без написания кастомного кода интеграции. Подробнее — в [Главе 18: Протоколы Инструментов и Tool Servers](../18-tool-protocols-and-servers/README.md).
...
A2A решает проблему интероперабельности: агенты от разных команд и на разных фреймворках взаимодействуют через единый протокол. Подробнее — в [Главе 18: Протоколы Инструментов и Tool Servers](../18-tool-protocols-and-servers/README.md).

### Почему этот курс учит с нуля

Этот курс строит агента с нуля по нескольким причинам:

1. **Понимание фундамента.** Фреймворк скрывает детали за абстракциями. Когда что-то ломается, вы не знаете, где искать. Написав agent loop, tool registry, memory store своими руками, вы понимаете каждый компонент.

2. **Осознанный выбор.** После реализации с нуля вы точно знаете, какие проблемы решает фреймворк. Решение «использовать LangGraph» или «написать кастом» становится обоснованным, а не случайным.

3. **Портабельность знаний.** Фреймворки меняются. Знание принципов (agent loop, Function Calling, context management) переносится на любой фреймворк. Знание конкретного API — нет.

4. **Go как явный язык.** Большинство фреймворков написаны на Python. Этот курс использует Go, что заставляет реализовывать паттерны явно — без «магии» декораторов и метапрограммирования.
...
1. **Working Memory критична.** Без неё агент теряет план и контекст прочитанных файлов между REPL-циклами.

2. **План = program state, не messages.** Хранение плана в Go-структуре позволяет ему пережить condensation. Хранение в messages — теряет план при суммаризации.

3. **Gradual degradation > hard cut.** 4-уровневое прогрессивное сжатие (trim → compact → condense → eviction) лучше, чем бинарная суммаризация "всё или ничего".

4. **VCR-тесты для агентов.** Записывание ответов LLM и их воспроизведение в тестах — единственный надёжный способ тестировать агентный цикл. Без этого — только manual testing.

5. **SHA256 loop detection — дёшево и надёжно.** Хеш от `(tool + input + output)` ловит циклы без LLM-вызовов.
...
**Сдано:**
- [x] Понимаете, когда использовать фреймворки vs кастомный runtime
- [x] Знаете, как избежать vendor lock-in через интерфейсы
- [x] Можете оценить фреймворки против ваших требований
- [x] Понимаете общие паттерны во фреймворках
- [x] Знаете реальные различия между production-агентами (Atlas vs Crush)
