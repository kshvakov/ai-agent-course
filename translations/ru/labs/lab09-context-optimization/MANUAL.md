...
Практический вывод: если инструмент возвращает большие данные (логи, длинный JSON, HTML), не кладите это целиком в `messages[]`. Сохраните результат как артефакт (например, в файл/объектное хранилище) и возвращайте модели только короткий excerpt + `artifact_id`. Так вы уменьшите стоимость и риск переполнения контекста, не теряя исходные данные.
