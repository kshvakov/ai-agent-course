...
### Прод-вариант: HITL как инструменты

В этой лабе мы делаем HITL через текст (CLI-режим). В проде часто удобнее вынести уточнения и подтверждения в отдельные инструменты:

- `ask_user` — структурные вопросы (single/multi select), чтобы ответы приходили как данные.
- `confirm_action` — явное подтверждение `write_local` / `external_action` с аудитом.

Идея остаётся той же: пока агент возвращает текст/запрос подтверждения — управление у пользователя.
