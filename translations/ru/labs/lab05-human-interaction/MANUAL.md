...
### Прод-паттерн: `ask_user` / `confirm_action` вместо "текстового ритуала"

В CLI достаточно текстового вопроса. В продукте обычно хочется, чтобы HITL был машиночитаемым и аудируемым. Тогда уточнения и подтверждения оформляют как отдельные инструменты:

- `ask_user` — вернуть форму с вопросами/вариантами, получить ответы структурно.
- `confirm_action` — запросить approval перед `write_local`/`external_action` и залогировать, кто подтвердил.

Это не меняет базовую механику nested loops: как только агент возвращает запрос к пользователю (вопрос/approval) — управление у пользователя.
